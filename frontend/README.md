...
- Jaspr web app (`apps/web`)
- Flutter app shell (`apps/mobile`)
- Shared domain layer for both (`packages/shared_core`)

## Why this structure
- Keep product logic in pure Dart (`core`) so both Jaspr and Flutter can reuse it.
...
2. From `frontend/dart_workspace/apps/web`, run `dart pub get` then `jaspr serve`.
3. From `frontend/dart_workspace/apps/mobile`, run `flutter pub get` then `flutter run`.
